...
int getDistance();
int changeLocation(int x, int y);
int ultrason(int x, int y);
...
{	
	...
	// asks the user to enter a distance for x
	displayCenteredBigTextLine(1, "Choose X Axis");
	getX = getDistance(); // finds the number for getX
	sleep(1000); // stops for 1 seconds
	
	// asks the user to enter a distance for y
	displayCenteredBigTextLine(1, "Choose Y Axis");
	getToY = getDistance(); // finds the number for getToY
	
	// displays that the robot is heading to the given coordinates
	displayCenteredBigTextLine(1, "Heading to [%d,%d]", getX, getToY);
	sleep(2000);
	
	// checks if x is bigger than the destination number and y is bigger
	if (x > getX && y > getToY) 
	{
		// to turn the robot left
		for (i = 0; i < 2; i++)
		{
			drive(20); // moves the robot forward
			x = x + 1;
			displayCenteredTextLine(4, "Current Position [%d,%d]", x, y); // displays the current position of the robot
			sleep(500);
		}
		
		// rotates the robot 270 degrees
		Angle_rotation(4, 2);
		
		// loop to move the robot forward 4 times
		for (i = 0; i < 4; i++)
		{
			drive(20); // moves the robot forward
			y = y - 1;
	...
	// checks if x is bigger than the destination number and y is smaller
	else if (x > getX && y <= getToY)
	{
		// to turn the robot left
...
void driveToPosition(int x, int X, int y, int Y, int key)
{
	...
	while (distance != 0)
	{
		...
		if (distance > 0) 
		{
			y = ultrason(x, y); //calling drive function to drive straight
			if (tempY == y) // checks if the robot is still in the same position
			{
			...
			// dislays the current position of the robot
	...
	dist = Y - y; // calculates the distance to the destination
	// loops while the distance is not 0
	while (dist != 0)
	{
		tempY = y;
...
int changeLocation(int x, int y) 
{
	...
	// loop to move the robot away from the object
	for (i = 0; i < 2; i++)
	{
		drive(20);
		y = y + 1;
	...
	// displays the new position of the robot
	displayCenteredTextLine(4, "New Position [%d,%d]", x, y);
	sleep(2000);
...
// function to check if the robot has detected an object
int ultrason(int x, int y) 
{
	// moves forward until a wall is detected
